...
// Set: panel structure
	...
	merge county_fips using "/Users/austinlw/Harris-Public-Policy Dropbox/Austin Wright/coronaScience/raw/out/essential_workers.dta"
...
coefplot (mBiS_High, label("Science Skepticism = Low") color(blue%75) recast(connected) ciopts(recast(rcap) color(blue%50))) (mBiS_Low, label("Science Skepticism = High") color(gray%75) recast(connected) ciopts(recast(rcap) color(gray%50))), ///
	...
	estout mBiS_High mBiS_Low using "${tabs}/county_eventstudy_noweights_$add.tex", ///
	replace cells((b(fmt(3))) (se(par fmt(3)) & t(fmt(2)))) style(tex) ///
	title(\caption{Event-Study Approach, Full Results} \label{fig:lockdown_response_table}) label ///
	prehead(\begin{tabular}{lcccc} \hline \hline) postfoot( \hline \end{tabular}) ///
	stats(r2 df_r N, fmt(3 0 1) label("R\textsuperscript{2}" "Degrees of Freedom" "Observations"))
