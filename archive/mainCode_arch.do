...
import delimited "${dir}/raw/out/archive/COVID_County.csv", clear
...
destring democrat  republican happening happeningoppose emp_manushare ///
	emp_hospshare sd_pcthome sd_allhomecount tweets ///
...
destring firstcase2, replace
by countyfips (date), sort: replace firstcase2 = firstcase2[_n-1] if firstcase2 >= . 
gsort countyfips -date 
by countyfips: replace firstcase2 = firstcase2[_n-1] if firstcase2 >= . 
...
gen dayssincecase2=dayssincecase
replace dayssincecase2=0 if dayssincecase<=0
...
gen trump = (republican > democrat)
gen dem = (republican < democrat)
...
foreach v of varlist DemSci RepSci DemNo RepNo { 
	...
	reghdfe y  ago0-ago$cut case0-case`daysmax' /// dat`datemin'-dat`datemax' alternative: ago0-ago$cut i.dayssincecase2 case0-case`daysmax'
		bus school jh_confirmed_s jh_deaths_s, ///
		absorb(i.ncounty i.date2) vce(cluster ncounty date2 i.nstate#i.week) nocons
...
collapse trump dem, by(ncounty)
egen totaltrump = sum(trump)
gen totaldem= _N-totaltrump
