...
// set paths
global dir "C:\Users\Valentin\Dropbox/coronaScience" // change this
...
import delimited "${dir}/raw/out/COVID_County_2.csv", clear
...
merge m:1 fips using "${dir}/raw/out/Vacc.dta"
...
gen vacc_diff = vacc_2018 - vacc_2012

// Dependent Variable
gen y = sd_pcthome

global clim = "vacc_diff" // which climate change variable
...
gen DemSci = ((dem==1) & ($clim <= r(p50)))
gen DemNo = ((dem==1) & ($clim > r(p50)))

quietly: sum $clim if trump==1, detail 
local medHapRep = r(p50)
gen RepSci = ((trump==1) & ($clim <= r(p50)))
gen RepNo = ((trump==1) & ($clim > r(p50)))

quietly: sum $clim, detail
local medSci = r(p50)
gen sciYes = ($clim < r(p50))
gen sciNo = ($clim > r(p50))
