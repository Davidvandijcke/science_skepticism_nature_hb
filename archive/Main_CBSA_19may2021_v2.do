...
global dir "~/Dropbox/coronaScience" 
...
// NB: chnaged the cutoff so we can add post-window dummy
...
// drop if date2 > 22006 | date2==. // trim sample to go until Apr 1 (sample period of original submission)
...
gen DemSci = .
replace DemSci= 1 if dem==1 & $clim <= `medHapDem'
replace DemSci= 0 if dem==1 & $clim > `medHapDem'

gen DemNo= .
replace DemNo= 1 if dem==1 & $clim > `medHapDem'
replace DemNo= 0 if dem==1 & $clim <= `medHapDem'

gen RepSci = .
replace RepSci= 1 if trump==1 & $clim <= `medHapRep'
replace RepSci= 0 if trump==1 & $clim > `medHapRep'

gen RepNo= .
replace RepNo= 1 if trump==1 & $clim > `medHapRep'
replace RepNo= 0 if trump==1 & $clim <= `medHapRep'
...
// gen the pre-window and post-window dummies (using 5 pre, 5 post)

summarize dayssincestate

gen pre_window=0
	replace pre_window=1 if dayssincestate>=-5&dayssincestate<0&dayssincestate!=.

gen post_window=0
	replace post_window=1 if dayssincestate>=21&dayssincestate<25&dayssincestate!=.
...
*case0-case$daysmax jh_confirmed_s jh_deaths_s 

// FOR DIAGNOSTICS: RUN THIS CHUNK AT TOP
...
foreach v of varlist $loopvars { // sciYes sciNo DemSci DemNo RepSci RepNo  or $loopvars
	...
	eststo m`v': reghdfe $fmla ago0-ago6 ago8-ago$cut pre_window if `v' == 1, ///
		absorb(i.ncounty i.ndate) /// 
		vce(cluster i.ncounty i.ndate i.nstate#i.week) nocons
	
}

coefplot msciYes msciNo, vertical keep(ago*) level(95)

coefplot mDemSci mDemNo, vertical keep(ago*) level(95)

coefplot mRepSci mRepNo, vertical keep(ago*) level(95)

eststo clear

gen esw_ry=dayssincestate-10

eventstudyweights pre_window ago0 ago1 ago2 ago3 ago4 ago5 ago6 ago7 ago8 ago9 ago10 ago11 ago12 ago13 ago14 ago15 ago16 ago17 ago18 ago19 ago20, controls(i.ncounty i.ndate) cohort(statedate) rel_time(esw_ry) saveweights("weights")

 import excel "weights.xlsx", clear firstrow
 
gen all=1
 
group_lines ago10 esw_ry statedate midblue%15 solid vthin myplot all 1 
...
foreach v of varlist $loopvars { // sciYes sciNo DemSci DemNo RepSci RepNo  or $loopvars
	...
	reghdfe $fmla ago0-ago$cut if post_window==0, ///
		absorb(i.ncounty i.ndate) /// 
		vce(cluster i.ncounty i.ndate i.nstate#i.week) nocons
...
foreach m of local mnames { 
	...
	twoway (scatter coeff2 xax, mcolor(black)  msymbol(X)) /// 
		...
		yscale(range(-0.02, 0.05)) ylabel(0(0.01)0.05))  
	...
	//// Democrats
	twoway (scatter coeff4 xax, mcolor(blue)  msymbol(X)) /// 
	...
	//// Republicans
	twoway (scatter coeff6 xax, mcolor(red)  msymbol(X)) /// 
		...
		xline(0, lc(cranberry)  lpattern(dash) lw(medthick)) ///
		yline(0.0, lpattern(dash) lc(black) lw(medthin))  ///
		yscale(range(-0.02, 0.05)) ylabel(0(0.01)0.05)), name(republicans`m')
...
// drop if aboveMedinstitutionalhealth==.| aboveMedreligion==.
...
reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc 1.dd#1.aboveMedreligion 1.dd#1.aboveMedinstitutionalhealth, absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons
...
reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump case0-case$daysmax soe bus school jh_confirmed jh_deaths jh_confirmed_s jh_deaths_s lconf ldeath 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc, absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons

reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump case0-case$daysmax soe bus school jh_confirmed jh_deaths jh_confirmed_s jh_deaths_s lconf ldeath 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc 1.dd#1.aboveMedreligion 1.dd#1.aboveMedinstitutionalhealth, absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons

reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump case0-case$daysmax soe bus school jh_confirmed jh_deaths jh_confirmed_s jh_deaths_s lconf ldeath 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc 1.dd#1.aboveMedreligion 1.dd#1.aboveMedinstitutionalhealth if pop!=., absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons

reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump case0-case$daysmax soe bus school jh_confirmed jh_deaths jh_confirmed_s jh_deaths_s lconf ldeath 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc 1.dd#1.aboveMedreligion 1.dd#1.aboveMedinstitutionalhealth [aweight=pop], absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons

gen DiD_sample=e(sample)



reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor 1.dd#1.aboveMedmdn_inc 1.dd#1.aboveMedreligion 1.dd#1.aboveMedinstitutionalhealth, absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons

reghdfe sd_pcthome dd 1.dd#1.aboveMedhuman 1.dd#1.trump 1.dd#1.aboveMedrural 1.dd#1.aboveMedbachelor, absorb(i.ncounty i.date2) vce(cluster i.ncounty i.date2 i.nstate#i.week) nocons
...
gen republican_pop=republican*pop

foreach var of varlist republican_pop pop {
bysort ncbsa date_smart: egen `var'_cbsa = sum(`var')
}

gen republican_share=republican_pop_cbsa/pop_cbsa

duplicates drop ncbsa date_smart, force // "collapse" panel to cbsa date -- no need to summarize cause already done in R

xtset ncbsa date_smart // set panel
...
gen firstcase_cbsa = date_smart if lconf_cbsa == 0 & jh_confirmed_cbsa == 1
bysort ncbsa: egen temp = min(firstcase_cbsa)
gen dayssincecase_cbsa = date_smart - temp
...
drop if sci_share_cbsa==. // |trump==.|dem==.

drop trump 

gen trump=0
	replace trump=1 if republican_cbsa>=.5
	replace trump=. if republican_cbsa==.

gen dem=0
	replace dem=1 if republican_cbsa<.5
	replace dem=. if republican_cbsa==.
...
foreach v of varlist $loopvars2 { // sciYes sciNo DemSci DemNo RepSci RepNo  or $loopvars
	...
	reghdfe $fmla ago0-ago$cut,  /// event study dummies
		absorb(i.ncbsa i.date_smart) vce(cluster i.ncbsa i.date_smart i.nstate#i.week) nocons
...
twoway (scatter coeff2 xax, mcolor(black)  msymbol(O)) /// above median: solid line ("[..]No")
	...
	xline(0, lc(cranberry)  lpattern(dash) lw(medthick)) ///
	yline(0.0, lpattern(dash) lc(black) lw(medthin)) ///
	yscale(range(-0.025, 0.05)) ylabel(0(0.01)0.05))  
...
twoway (scatter coeff6 xax, mcolor(red)  msymbol(O)) /// 
	...
	xline(0, lc(cranberry)  lpattern(dash) lw(medthick)) ///
	yline(0.0, lpattern(dash) lc(black) lw(medthin))  ///
	yscale(range(-0.025, 0.05)) ylabel(0(0.01)0.05)), name(republicans`m')
